...
Los sistemas fotovoltaicos modernos presentan vulnerabilidades de ciberseguridad debido a la ausencia de mecanismos nativos robustos o se suelen omitir en la practica como lo es el cifrado y la autentificación de protocolos de comunicación como Modbus TCP y MQTT. Esta condición se se complica debido a las limitaciones metodológicas y regulatorias de ejecutar ataques dirigidos a entornos de producción real, esto genera vacíos críticos de seguridad en datos operativos y firmas de tráfico anómalo para entrenamiento de sistemas de detección.

Ante este escenario, cualquier solución tecnológica debe someterse a procesos de validaciones objetivas y científicas para que la implementación trascienda de lo simple a lo técnico. ES imperativo poder determinar con gran precisión cuales son los componentes que serán evaluados y poder definir de manera cualitativa y cuantitativa la evidencia empírica que pueda comprobar la viabilidad y utilidad de la propuesta real, garantizando que el diseño propuesto responda con fidelidad a las problemáticas de los sistemas fotovoltaicos en términos de ciberseguridad.

Por consiguiente, el sistema propuesto se constituye de un honeypot de media interacción en una Raspberry Pi 4, un módulo de simulación físico-matemático que describa una planta fotovoltaica real y un clasificador basado en Machine Learning. Será evaluado a partir de preguntas de investigación y hipótesis de trabajo bien delimitadas. Este enfoque metodológico permitirá auditar en la credibilidad del entorno propuesto como señuelo para posibles atacantes como de tener la capacidad del modelo para poder diferenciar entre tráfico legitimo y malicioso.

Con este propósito, el presente documento define formalmente las preguntas de investigación, las hipótesis de trabajo y la matriz de variables preliminares que orienten al proyecto a un desarrollo experimental y a la posterior validación como tesis de grado.

## 2. Preguntas de investigación.

#### Pregunta general.

¿De que manera el diseño, implementación e integración de un honeypot industrial de media interacción integrado con simulación matemático-solar y un modulo de Machine Learning permite detectar y clasificar eventos maliciosos dirigidos a sistemas fotovoltaicos y entornos SCADA?

#### Preguntas especificas.

¿En qué medida es posible generar un entorno fotovoltaico simulado con características operativas realistas que apoye la captura de eventos de ciberseguridad y la generación de datos útiles para el análisis de amenazas?

¿Cual es el nivel de precisión, recall y puntuación F1 del algoritmo Random Forest al clasificar los eventos capturados por el honeypot en las categorías de tráfico predefinidas?
...
¿En qué medida la telemetría generada matemáticamente mantiene coherencia con el comportamiento esperado de una planta fotovoltaica cuando es expuesta mediante servicios Modbus TCP y MQTT?
...
El método consta en poder vincular el modelo matemático-solar y los datos históricos a servicios como lo son el Modbus TCP y MQTT los cuales están expuestos en la raspberry Pi 4, con esto se espera lograr que la desviación de las variables obtenidas sea menor al 5% y asi lograr un entorno señuelo credible y útil para la captura de ataques cibernéticos.

La justificación la encontramos que al abrir servicios específicos como lo son los OT e IoT, los ataques dejen de ser ruido genérico y se puedan convertir en eventos con marcas de tiempo, credenciales y comandos específicos del sector eléctrico solar.

#### Hipótesis 2.

Si se entrena un algoritmo de Machine Learning basado en Random Forest y que este utilice características tabulares que son extraídas de el historial de los logs del sistema el sistema logrará alcanzar una precisión, recall y puntuación F1 superior al 90% al clasificar el tráfico y eventos legítimos t maliciosos definidos para el estudio.

Como método se tiene el entrenamiento del clasificador Random Forest con los datos etiquetados de tráfico normal y malicioso del sistema, con esto se espera un resultado de las métricas de rendimiento sean superiores al 90% sobre el conjunto de prueba independiente.

Como justificación se tiene que los arboles de decisión del algoritmo manejan de forma excelente las fronteras de los datos tabulares cada que un ataque cambia drásticamente los umbrales de la red.
...
El método es desplegar y monitorear de forma centralizada a superficie de ataque multi-servicio (puertos 80, 22, 502 y 1883), con esto poder lograr resultados de exfiltraciones exitosas de vectores que cumplan con las características suficientes para la construcción del dataset.
...
#### SSH
Del servicio SSH se extraerán las siguientes variables: número de intentos de autenticación realizados durante la sesión, número de comandos ejecutados una vez establecida la conexión, duración total de la sesión en segundos, y resultado de autenticación codificado como variable categórica que distinguirá entre credencial válida o inválida, usuario existente o inexistente, y credencial por defecto o personalizada. Esta codificación permitirá al modelo identificar patrones de acceso asociados al uso de credenciales débiles o predeterminadas, frecuentes en ataques dirigidos a equipos industriales.
...
La variable dependiente corresponde a la clase o etiqueta que el modelo deberá asignar a cada sesión analizada. Se definen cinco clases mutuamente excluyentes que representan los tipos de evento que el sistema de detección de intrusiones deberá ser capaz de identificar:
